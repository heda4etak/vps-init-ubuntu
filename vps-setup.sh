...
main() {
  ...
  read -p " - ⚠️Нажмите Enter для начала установки или Ctrl+C для отмены..."
  ...
  read -p "$(echo -e \" - ⚠️ ${YELLOW}Нажмите Enter для перезапуска системы или Ctrl+C для ручной перезагрузки позже.${RESET}\\n\")"
  sudo reboot
}
